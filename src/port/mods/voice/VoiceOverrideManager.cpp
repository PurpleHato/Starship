...
static bool DecodeWav(std::shared_ptr<Ship::File> file, VoiceOverrideData& out) {
    ...
    SPDLOG_INFO("[VoiceOverride] WAV header: sampleRate={}, channels={}, bitsPerSample={}, frames={}", wav.sampleRate,
                wav.channels, wav.bitsPerSample, numFrames);
    SPDLOG_INFO("[VoiceOverride] WAV raw: fileSize={} bytes", FileSize(file));
    ...
    SPDLOG_INFO("[VoiceOverride] WAV decoded: {} samples, {} bytes PCM, {:.3f}s duration", out.numSamples, out.size,
                duration);
    return true;
}
...
static long VorbisTell(void* src) {
...
static bool LoadVoiceOverride(uint32_t msgId) {
    ...
    for (auto ext : extensions) {
        ...
        SPDLOG_INFO("[VoiceOverride] Loaded file for msgId={}, {} bytes from archive '{}', decoding as {}", msgId,
                    FileSize(file), GetArchiveName(path), ext);

        bool ok = false;
        if (strcmp(ext, "wav") == 0)
            ok = DecodeWav(file, data);
        else if (strcmp(ext, "mp3") == 0)
            ok = DecodeMp3(file, data);
        else if (strcmp(ext, "ogg") == 0)
            ok = DecodeOgg(file, data);

        if (ok && data.sampleData != nullptr && data.numSamples > 0) {
            float freqMod = (float) (data.sampleRate * data.channels) / 32000.0f;
            SPDLOG_INFO(
                "[VoiceOverride] Decode {} for msgId={} OK: {}Hz, {}ch, {} samples, {} bytes PCM, freqMod={:.4f}", ext,
                msgId, data.sampleRate, data.channels, data.numSamples, data.size, freqMod);
...
void VoiceOverride_Init(void) {
    ...
    for (auto& path : *files) {
        ...
        SPDLOG_INFO("[VoiceOverride] Registered msgId={} from archive='{}' path='{}'", msgId, GetArchiveName(path),
                    path);
    }
}
